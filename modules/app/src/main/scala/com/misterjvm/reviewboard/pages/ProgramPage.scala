...
object ProgramPage {
  ...
  def render(program: Program, reviewsSignal: Signal[List[Review]]) = List(
    ...
    div(
      cls := "container-fluid",
      renderProgramSummary(program), // TODO: fill summary later
  ...
  def renderProgramSummary(program: Program) = {
    ...
    div(
      ...
      div(
        ...
        div(
          cls := "program-description review-summary-contents",
          child <-- summaryBus.events.map(
            _.map(_.contents)
              .getOrElse("No generated review yet.")
          )
        ),
        child.maybe <-- summaryBus.events.map(
          _.map(_.created)
            .map(time => s"Generated by GPT-4 ${Time.unix2hr(time.toEpochMilli())}")
